...
foreach($nodes as &$node){
	...
	if(!$filtered){
		echo "<hr/><table cellpadding='0' cellspacing='0'><tr><td valign='center'><img src='https://www.gravatar.com/avatar/".md5($node[0])."?d=retro&f=y&s=32'>&nbsp&nbsp</td><td valign='center'>{$node[0]}<br/><span class='small'>".time_elapsed_string("@".$node[1])."</span></td></tr></table><div style='margin-top:4px; margin-bottom:4px;'>".str_replace("\n","<br/>",str_replace("<","<x><</x>",$node[2]))."</div><form method='POST'><input name='horpost_like_target' value='{$node[4]}' hidden><input class='micro grey' type='submit' name='horpost_like' value='{$likebtn} (".count($node[3]).")'></form>";
	}
}
?>
